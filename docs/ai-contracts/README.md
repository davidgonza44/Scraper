...
OpenSpec remains authoritative. Schemas describe public contracts, not
implementation internals. Fixtures are golden examples of those contracts.
...
3. Read architecture docs under `docs/architecture/` when present.
...
is the search-session core. Later stages are semantic fixtures, not proof that
the corresponding PR is implemented.
