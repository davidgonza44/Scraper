...
CSV SHALL contain one row per real listing in each provider's frozen canonical session result prefix. Extra usable candidates remaining in the acquisition buffer SHALL NOT be exported. After Z4, Alibaba REVIEW and IRRELEVANT listings SHALL NOT be exported. CSV SHALL NOT merge positionally aligned candidates into a single identity record, and presentation projections SHALL neither remove nor add canonical session results. Marketplace identity SHALL remain explicit.
